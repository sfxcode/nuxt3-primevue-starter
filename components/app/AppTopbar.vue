...
<template>
  <div class="layout-topbar">
    ...
    <ul class="layout-topbar-menu hidden lg:flex origin-top">      
      ...
      <li>
        <button class="p-link layout-topbar-button" @click="toggleSettingOverlay">
          <i class="pi pi-cog" />
          <span>Settings</span>
        </button>
      </li>
    ...
    <OverlayPanel id="overlay_panel" ref="userOverlay" append-to="body" style="width: 200px" class="text-center">      
      <Button label="SignOut" class="p-button-raised p-button-text p-button-link"
        @click="keycloak.logout()"></Button>
    </OverlayPanel>
    <client-only>
      <OverlayPanel id="overlay_panel" ref="op" append-to="body" style="width: 200px">
        <div class="field-radiobutton">
          <RadioButton id="saga" v-model="themeStore.themeName" name="layoutColorMode" value="saga"
            @change="themeStore.setLight()" />
          <label>Light</label>
        </div>
        <div class="field-radiobutton">
          <RadioButton id="vela" v-model="themeStore.themeName" name="layoutColorMode" value="vela"
            @change="themeStore.setDim()" />
          <label>Dim</label>
        </div>
        <div class="field-radiobutton">
          <RadioButton id="arya" v-model="themeStore.themeName" name="layoutColorMode" value="arya"
